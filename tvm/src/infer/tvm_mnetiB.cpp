...
tvm_mneti::tvm_mneti(std::string path, std::string name, std::string hardware, 
            int w, int h, bool with_blur, int batch, int mode, int devid):
...
void tvm_mneti::detect(cv::Mat& im, std::vector<cv::Rect2f>  & target_boxes,
    ...
    for (int index = 0; index < 3; ++index)
    {
        ...
        */
        ...
        if(with_blur_)
            tensor_slice(blur_scores1, blur_scores2, idx, 1);
        boxes.insert(boxes.end(), boxes2.begin(), boxes2.end());
        if(with_blur_)
            blur_scores.insert(blur_scores.end(), blur_scores2.begin(), blur_scores2.end()); 
    ...
    if(with_blur_)
        sort_with_idx(blur_scores,  order_blur_scores, order, 1);
    ...
    if(with_blur_)
        tensor_slice(order_blur_scores,     target_blur_scores,     keep, 1);
